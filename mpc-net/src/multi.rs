...
pub struct InMemoryTestNet {
    nodes: HashMap<usize, MpcNetConnection<DuplexStream>>,
}
...
fn new_connection<IO: AsyncRead + AsyncWrite + Unpin>(
    party_id: usize,
    n_parties: usize,
) -> MpcNetConnection<IO> {
    let mut connection = MpcNetConnection {
        id: party_id as u32,
        listener: None,
    ...
    for peer_id in 0..n_parties {
        connection.peers.insert(
            peer_id as u32,
            Peer {
                id: peer_id as u32,
                listen_addr: dummy_peer_addr(),
                streams: None,
            },
        );
    }
...
impl InMemoryTestNet {
    pub async fn new_local_testnet(
        n_parties: usize,
    ) -> Result<Self, MpcNetError> {
        let mut nodes = (0..n_parties)
            .map(|party_id| {
                (
                    party_id,
                    new_connection::<DuplexStream>(party_id, n_parties),
                )
            })
            .collect::<HashMap<_, _>>();

        for client_id in 0..n_parties {
            for server_id in client_id + 1..n_parties {
                let (client_stream, server_stream) =
                    tokio::io::duplex(16 * 1024 * 1024);
                let (client_mux, server_mux) = tokio::try_join!(
                    multiplex_stream(MULTIPLEXED_STREAMS, false, client_stream),
                    multiplex_stream(MULTIPLEXED_STREAMS, true, server_stream)
                )?;
                ...
                nodes
                    ...
                    .unwrap()
                    .peers
                    .get_mut(&(server_id as u32))
                    .unwrap()
                    .streams = Some(client_mux);
                nodes
                    .get_mut(&server_id)
                    .unwrap()
                    .peers
                    .get_mut(&(client_id as u32))
                    .unwrap()
                    .streams = Some(server_mux);
            }
        }
    ...
    >(
        self,
        user_data: U,
        f: impl Fn(MpcNetConnection<DuplexStream>, U) -> F
            + Send
            + Sync
            + Clone
            + 'static,

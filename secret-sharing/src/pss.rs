...
impl<F: FftField> PackedSharingParams<F> {
    ...
    pub fn new(l: usize) -> Self {
        let n = l * 4;
        let t = l;
        Self::new_unchecked(n, l, t)
    }

    /// Creates a new instance with explicit `(n, l, t)` parameters.
    ///
    /// The current implementation uses radix-2 evaluation domains, so `n`,
    /// `l + t`, and `2(l + t)` must all be powers of two. We also require
    /// enough share points to reconstruct degree-doubled sharings.
    pub fn new_with_params(n: usize, l: usize, t: usize) -> Self {
        assert!(n.is_power_of_two(), "n must be a power of two");
        assert!((l + t).is_power_of_two(), "l + t must be a power of two");
        assert!(
            2 * (l + t) <= n,
            "n must be at least 2(l + t) for multiplied shares"
        );
        assert!(t < n / 2 - l, "parameters must satisfy t < n/2 - l");
        Self::new_unchecked(n, l, t)
    }

    fn new_unchecked(n: usize, l: usize, t: usize) -> Self {
...
mod tests {
    ...
    #[test]
    fn test_safe_explicit_params_16_3_1() {
        let pp = PackedSharingParams::<F>::new_with_params(16, 3, 1);
        assert_eq!(pp.t, 1);
        assert_eq!(pp.l, 3);
        assert_eq!(pp.n, 16);

        let rng = &mut ark_std::test_rng();
        let secrets: [F; 3] = UniformRand::rand(rng);
        let secrets = secrets.to_vec();

        let shares = pp.pack(secrets.clone(), rng);
        assert_eq!(secrets, pp.unpack(shares.clone()));

        let expected: Vec<F> = secrets.iter().map(|x| (*x) * (*x)).collect();
        let mul_shares: Vec<F> = shares.iter().map(|x| (*x) * (*x)).collect();
        assert_eq!(expected, pp.unpack2(mul_shares.clone()));

        let lagrange_secrets = pp.lagrange_unpack(
            &mul_shares[0..pp.n - pp.t + 1],
            &(0..(pp.n - pp.t + 1) as u32)
                .collect::<Vec<u32>>()
                .as_slice(),
        );

...
impl<F: PrimeField, D: EvaluationDomain<F> + Send> QAP<F, D> {
    pub fn pss(
        ...
        let mut pack = |label: &str, mut x: Vec<F>| {
            let start = Instant::now();
            fft_in_place_rearrange(&mut x);
            println!(
                "  │  ├─ qap.pss {} fft/rearrange: len={}, {:.2}s",
                label,
                x.len(),
            ...
            let m = x.len();
            let chunk_count = m / pp.l;
            pevals.reserve(chunk_count);
            for i in 0..chunk_count {
                let secrets = (0..pp.l)
                    .map(|j| x[i + j * chunk_count])
                    .collect::<Vec<_>>();
                pevals.push(pp.pack(secrets, rng));
            }
            println!(
                "  │  └─ qap.pss {} pack: {} chunks, {:.2}s",
                label,
                pevals.len(),
                start.elapsed().as_secs_f64()
            );
        ...
        let packed_a = pack("a", self.a.clone());
        let packed_b = pack("b", self.b.clone());
        let packed_c = pack("c", self.c.clone());
